...
eval :: Dict -> Prog -> Dict
eval dict (PlusPlus str) = ins str ((valueOf str dict)+1) dict
eval dict (str :=: e) = ins str (eval_pexpr dict e) dict
eval dict (DeclareInt str) = dict -- ins str 0 dict
eval dict (Begin p1 p2) = eval (eval dict p1) p2
eval dict (While b p) = eval dict p
eval dict (If b p1 p2) = eval (eval dict p1) p2
